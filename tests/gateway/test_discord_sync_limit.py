...
def adapter():
    ...
    _ensure_discord_mock()
    config = PlatformConfig(platform="discord", name="discord", token="fake-token")
...
async def test_safe_sync_deletes_before_creating():
    ...
    # Desired state: remove cmd_0, keep cmd_1..cmd_99, add cmd_new
    desired_dict = {
        (1, f"cmd_{i}"): {"name": f"cmd_{i}", "type": 1}
        for i in range(1, 100)
    }
    desired_dict[(1, "cmd_new")] = {"name": "cmd_new", "type": 1}

    adapter._client.tree.get_commands = MagicMock(
        return_value=[
            SimpleNamespace(name=f"cmd_{i}", type=1)
            for i in range(1, 100)
        ] + [SimpleNamespace(name="cmd_new", type=1)]
    )
